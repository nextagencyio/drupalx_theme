describe('Pricing Component', () => {
  ...
  it('renders the default pricing section with correct content', () => {
    ...
    cy.get('h1').should('contain', 'Compare Our Options');
    ...
    // Check number of cards using specific classes from the template
    cy.get('.grid > .rounded-xl.bg-card.text-card-foreground').should('have.length', 3);
  });

  it('displays correct content for each pricing card', () => {
    // Check first card (Free plan)
    cy.get('.grid > .rounded-xl.bg-card').eq(0).within(() => {
      cy.get('h2').should('contain', 'DrupalX CMS');
      cy.get('h3').should('contain', 'Free');
      cy.get('.grid.grid-cols-1 > div').should('have.length', 3);
      cy.get('a').should('contain', 'Get Started').and('have.attr', 'href', '#');
    });
    ...
    cy.get('.grid > .rounded-xl.bg-card').eq(1).within(() => {
      ...
      cy.get('.grid.grid-cols-1 > div').should('have.length', 3);
      cy.get('a').should('contain', 'Book Discovery').and('have.attr', 'href', '#');
    });
    ...
    cy.get('.grid > .rounded-xl.bg-card').eq(2).within(() => {
      ...
      cy.get('.grid.grid-cols-1 > div').should('have.length', 3);
      cy.get('a').should('contain', 'Contact Sales').and('have.attr', 'href', '#');
    });
  });
  ...
  it('verifies features list in each card', () => {
    ...
    cy.get('.grid > .rounded-xl.bg-card').eq(0).within(() => {
      cy.get('.grid.grid-cols-1 > div').eq(0).find('p').should('contain', 'Full access to open source features');
      cy.get('.grid.grid-cols-1 > div').eq(1).find('p').should('contain', 'Community support');
      cy.get('.grid.grid-cols-1 > div').eq(2).find('p').should('contain', 'Documentation');
    });

    // Check features in second card
    cy.get('.grid > .rounded-xl.bg-card').eq(1).within(() => {
      cy.get('.grid.grid-cols-1 > div').eq(0).find('p').should('contain', 'Comprehensive needs assessment');
      cy.get('.grid.grid-cols-1 > div').eq(1).find('p').should('contain', 'Custom solution design');
      cy.get('.grid.grid-cols-1 > div').eq(2).find('p').should('contain', 'Implementation roadmap');
    });
  });

  describe('Two Cards Layout', () => {
    beforeEach(() => {
      // Visit the two cards story variant
    ...
    it('displays correct content for two-card layout', () => {
      ...
      cy.get('h1').should('contain', 'Compare Plans');
      ...
      cy.get('.grid > .rounded-xl.bg-card').eq(0).within(() => {
        ...
        cy.get('h3 span').should('contain', '/mo');
        cy.get('.grid.grid-cols-1 > div').should('have.length', 3);
