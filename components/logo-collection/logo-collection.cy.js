describe('Logo Collection Component', () => {
  ...
  it('renders the default story with correct title and logos', () => {
    ...
    cy.get('.container .flex-wrap > div').should('have.length', 3);
  ...
  it('renders logos with correct max-width class', () => {
    cy.get('img').each(($img) => {
      cy.wrap($img).should('have.class', 'max-w-[120px]');
    });
  });
  ...
  context('Few Logos Variant', () => {
    ...
    it('renders with fewer logos correctly', () => {
      ...
      cy.get('.container .flex-wrap > div').should('have.length', 2);
  ...
  context('Many Logos Variant', () => {
    ...
    it('renders with additional logos correctly', () => {
      ...
      cy.get('.container .flex-wrap > div').should('have.length', 6);
      cy.get('img').should('have.length', 6);
    });
  });

  it('verifies logo spacing and alignment', () => {
    cy.get('.flex-wrap').should('have.class', 'gap-4');
    cy.get('.flex-wrap').should('have.class', 'md:gap-8');
    cy.get('.flex-wrap').should('have.class', 'justify-start');
    cy.get('.flex-wrap').should('have.class', 'md:justify-end');
  });
